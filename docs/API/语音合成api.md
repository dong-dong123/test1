...
| | | | | \
|`wss://openspeech.bytedance.com/api/v1/tts/ws_binary` |**不推荐，​**WebSocket协议，一次性输入合成文本，流式输出音频。 |语音合成、声音复刻、不支持混音 |[V1 WebSocket单向流式接口文档](https://www.volcengine.com/docs/6561/1257584) |
| | | | | \
|`https://openspeech.bytedance.com/api/v1/tts` |**不推荐，​**HTTP协议，一次性输入合成文本，一次性输出音频。 |语音合成、声音复刻、不支持混音 |[V1 HTTP非流式接口文档](https://www.volcengine.com/docs/6561/1257584) |
...
| | | | | | | \
|cluster |业务集群 |2 |string |✓ |volcano*tts |
| | | | | | | \
|user |用户相关配置 |1 |dict |✓ | |
| | | | | | | \
...
| | | | | |_ `en` 仅英文 |
| | | | | | | \
|context*language |参考语种 |2 |string | |给模型提供参考的语种 |\
| | | | | | |\
| | | | | |* 不给定 西欧语种采用英语 |\
| | | | | |_ id 西欧语种采用印尼 |\
| | | | | |_ es 西欧语种采用墨西 |\
...
| | | | | | | \
|latex_parser |\
| | |3 |string | |是否使用lid 能力播报latex公式，相较于latex_tn 效果更好； |\
