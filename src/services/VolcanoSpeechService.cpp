...
bool VolcanoSpeechService::loadConfig()
{
    ...
    ESP_LOGI(TAG, "Loaded Volcano config: Endpoint=%s, Region=%s, Language=%s, Voice=%s",
             config.endpoint.c_str(), config.region.c_str(), config.language.c_str(), config.voice.c_str());
...
bool VolcanoSpeechService::callWebSocketRecognitionAPI(const uint8_t* audio_data, size_t length, String& text)
{
    ...
    ESP_LOGI(TAG, "Connecting to WebSocket non-stream recognition API: %s", NOSTREAM_RECOGNITION_API);
    webSocketClient->setExtraHeaders(headers);

    if (!webSocketClient->connect(NOSTREAM_RECOGNITION_API))
    {
    ...
    request["enable_punc"] = config.enablePunctuation;
    request["enable_itn"] = true;
    request["enable_ddc"] = false;

    String requestJson;
    serializeJson(requestDoc, requestJson);

    ESP_LOGI(TAG, "Sending full client request: %s", requestJson.c_str());

    // 注意：根据API文档，full client request应该是二进制消息
    // 但这里先使用文本消息测试
    if (!webSocketClient->sendText(requestJson))
    {
    ...
    // 发送audio only request（音频数据）
    // 根据API文档，audio only request是二进制消息
    // 简化：直接发送二进制数据
    ESP_LOGI(TAG, "Sending audio data (%u bytes) as binary message", length);
    if (!webSocketClient->sendBinary(audio_data, length))
    {
    ...
    // 检查是否有响应消息（简化处理）
    // 这里应该从回调中获取响应，但为了简单，直接返回假
    lastError = "WebSocket recognition not fully implemented - response handling needed";
...
bool VolcanoSpeechService::callStreamRecognitionEnd(String &final_text)
{
    ...
    if (!webSocketClient->sendText(endJson))
    {
...
bool VolcanoSpeechService::sendWebSocketAuth()
{
    ...
    if (!webSocketClient->sendText(authJson))
    {
...
bool VolcanoSpeechService::sendWebSocketRecognitionStart()
{
    ...
    if (!webSocketClient->sendText(startJson))
    {
...
void VolcanoSpeechService::parseWebSocketMessage(const String &jsonMessage)
{
